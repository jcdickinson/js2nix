...
class Package {
  ...
  static create(
    ...
    if (resolved && resolved.startsWith("https")) {
      ...
      if (integrity && integrity.startsWith("sha1-")) {
        src = { 
          sha1: integrity.substr(5),
          url: parsedUrl.origin + parsedUrl.pathname, 
          type: 'url',
        };
      } else if (integrity && integrity.startsWith("sha256-")) {
        src = { 
          sha256: integrity.substr(7),
          url: parsedUrl.origin + parsedUrl.pathname, 
          type: 'url',
        };
      } else if (integrity && integrity.startsWith("sha512-")) {
        src = { 
          sha512: integrity.substr(7),
          url: parsedUrl.origin + parsedUrl.pathname, 
          type: 'url',
        };
      } else if (['registry.yarnpkg.com', 'registry.npmjs.org'].includes(parsedUrl.host)) {
        src = { 
  ...
  renderSrc() {
    switch (this.src.type) {
      case "url":
        const { url, sha1, sha256, sha512 } = this.src;
...
function generateDepTree(lock) {
  ...
  const shortRefs = Object.entries(pkgs).reduce((acc, [key, value]) => {
    ...
    if (acc[short]) {
      const keys = BrokenReference.is(acc[short])
        ? Array.from(acc[short].keys)
        : [acc[short].key];
      return {
        ...acc,
...
function hostDependencyCycles(tree) {
  ...
  function traverse(node, tp = []) {
    ...
    const found = tp.find(
      (key) => key.split(hostDelimiter).pop() === node.key.toString(),
    );
    if (found) {
  ...
  function hostCycle(cycle) {
    assert.ok(
      cycle.length > 1,
      "Unable to handle a cycle with a length less that 2",
    );
...
${Object.keys(tree)
  ...
  ${Object.keys(tree)
    .sort()
    .filter(
      (curr) => Reference.is(tree[curr]) && !BrokenReference.is(tree[curr]),
    )
